...
export async function scoreJobWithAi(
  ...
  try {
    const { data, model } = await requestJson<{
      ...
      system:
        "你是 JobLoop 的岗位评分与投递决策助手。请基于结构化 JD、公司补充信息和简历版本，对单个岗位打分并输出求职决策摘要。评分体系固定：行业匹配度25%、公司实力20%、岗位匹配度30%、薪资竞争力15%、成长性10%。每个维度先单独给0-100分，再按权重计算总分。\n\n薪资竞争力评分规则：\n- 以薪资区间最低值为基准，必须结合 JD 中的工作地点（location）和工作年限（experienceRequirement）综合评估，不得仅凭绝对数值判断。\n- 城市分级参考：一线（北上广深）基准最高，新一线（杭州、成都、武汉、南京等）次之，二三线依次降低。若 location 缺失或模糊，默认按新一线标准。\n- 年限分级参考：1-3年→初级，3-5年→中级，5-10年→高级，10年+→资深/专家。\n- 综合判断：同城同级下，最低薪资达到同行中上水平即为高竞争力。示例：深圳 1-3年 15K+ → 高(≥80)，10-15K → 中等(50-70)，<10K → 低(<50)。示例：成都 3-5年 18K+ → 高(≥80)，12-18K → 中等(50-70)，<12K → 低(<50)。\n- 若 JD 缺少薪资信息，按中性分50处理并在 summary 中说明。\n- 若 JD 缺少城市或年限信息，在 summary 中注明\u201C因缺少XX信息，薪资竞争力评估可能偏差\u201D。\n\n只返回合法 JSON。",
      prompt: {
