...
当前 P0 已从早期求职过程管理方向收敛为“投递前决策工作台”。当前实现已完成到 `specs/001-jobloop-p0/tasks.md` 的主路径收尾：简历库、基础简历生成、岗位分析工作台、单岗位详情、公司补充信息、岗位微调和 AI 输出追溯都已接入本地持久化。
...
当前实现已接入 OpenRouter 的 OpenAI-compatible 服务端调用；模型通过环境变量配置，默认走 `deepseek` 路线。
...
P0 保留“公司补充信息，可选”输入框，让用户手动粘贴公司背景、风评、融资、团队或岗位真实性线索。界面保留“联网补充公司信息 Beta / Coming soon”入口，但核心分析不得依赖真实联网搜索。
...
- 数据暂存于浏览器 `localStorage`，键名为 `jobloop:p0-state`。
- 批量 JD 解析支持空行、`---` 或 `###` 分隔。
- 公司信息是可选输入；联网补充按钮只显示 Beta / Coming soon，不触发真实搜索。
- 一级导航只保留：入口、简历库、岗位分析。
